 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto it : llvm::zip(adaptor.init_values(), adaptor.out())) {
      Value init_value =
          rewriter.create<memref::LoadOp>(loc, std::get<0>(it));
    ...
    size_t operand_num = reduce_op.getOperation()->getNumOperands() / 3;
    SmallVector<ArrayRef<AffineExpr>, 4> affine_maps;
    affine_maps.insert(affine_maps.end(), operand_num, makeArrayRef(src_exprs));
    affine_maps.insert(affine_maps.end(), operand_num, makeArrayRef(dst_exprs));
                                ...
                                linalg_op.region().end());
    {
      ...
      for (size_t i = 0; i < block->getNumArguments(); ++i) {
        mem_argv_tys.push_back(block->getArgumentTypes()[i].cast<MemRefType>());
        alloc_values.push_back(
            rewriter.create<memref::AllocaOp>(loc, mem_argv_tys.back()));
      }
      size_t input_num = alloc_values.size() / 3 * 2;
      ...
      SmallVector<Type, 4> element_tys;
      for (size_t i = 0; i < input_num; ++i)
        element_tys.push_back(mem_argv_tys[i].getElementType());

      signature_converter.addInputs(element_tys);
      ...
      for (auto it :
           enumerate(makeArrayRef(alloc_values).slice(0, input_num))) {
      ...
      auto output_values = makeArrayRef(alloc_values).slice(input_num);
